...
`timelockcoin_xx_YY.ts or timelockcoin_xx.ts`

`src/qt/locale/timelockcoin_en.ts` is treated in a special way. It is used as the source for all other translations. Whenever a string in the source code is changed, this file must be updated to reflect those changes. A custom script is used to extract strings from the non-Qt parts. This script makes use of `gettext`, so make sure that utility is installed (ie, `apt-get install gettext` on Ubuntu/Debian). Once this has been updated, `lupdate` (included in the Qt SDK) is used to update `timelockcoin_en.ts`.

To automatically regenerate the `timelockcoin_en.ts` file, run the following commands:
```sh
cd src/
make translate
```

`contrib/timelockcoin-qt.pro` takes care of generating `.qm` (binary compiled) files from `.ts` (source files) files. It’s mostly automated, and you shouldn’t need to worry about it.
...
```
git add src/qt/timelockcoinstrings.cpp src/qt/locale/timelockcoin_en.ts
git commit
```
...
3. Update `src/qt/timelockcoin_locale.qrc` manually or via
```bash
...
1. Open `timelockcoin_en.ts` in Qt Linguist (included in the Qt SDK)
...
To create a new language template, you will need to edit the languages manifest file `src/qt/timelockcoin_locale.qrc` and add a new entry. Below is an example of the English language entry.

```xml
<qresource prefix="/translations">
    <file alias="en">locale/timelockcoin_en.qm</file>
    ...
</qresource>
```
