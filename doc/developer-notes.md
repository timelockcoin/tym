...
Run with the `-testnet` option to run with "play TYMs (tTYM)" on the test network, if you
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_timelockcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_timelockcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/timelockcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_timelockcoin.coverage/index.html`.
```
...
- BitcoinMiner : Generates TYMs (if wallet is enabled).
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `timelockcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `timelockcoin-cli` and the GUI debug console use this table to determine how to
