...
*Setup instructions for a gitian build of TimelockCoin Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the TimelockCoin
...
to the TimelockCoin GitHub release page.
...
- [Building TimelockCoin Core](#building-timelockcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/timelockcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/timelockcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/timelockcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/timelockcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/timelockcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/timelockcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/timelockcoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@timelockcoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/timelockcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/timelockcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/timelockcoin-osx-*-build.assert
```
