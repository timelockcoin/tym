...
void ThreadFlushWalletDB(const std::string& strFile)
{
    ...
    RenameThread("timelockcoin-wallet");
...
bool CWalletDB::WriteZTYMSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZTYMSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZTYMSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZTYMSeed_deprecated()
{
    return Erase(std::string("dzs"));
}

bool CWalletDB::ReadZTYMSeed(const uint256& hashSeed, std::vector<unsigned char>& seed)
{
    return Read(std::make_pair(std::string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZTYMSeed_deprecated(uint256& seed)
{
    return Read(std::string("dzs"), seed);
}

bool CWalletDB::WriteZTYMCount(const uint32_t& nCount)
{
    return Write(std::string("dzc"), nCount);
}

bool CWalletDB::ReadZTYMCount(uint32_t& nCount)
{
