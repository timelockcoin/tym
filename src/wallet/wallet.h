...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 TYM at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zTYM send
enum ZerocoinSpendStatus {
    ZTYM_SPEND_OKAY = 0,                            // No error
    ZTYM_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZTYM_WALLET_LOCKED = 2,                         // Wallet was locked
    ZTYM_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZTYM_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZTYM_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZTYM_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZTYM_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZTYM_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZTYM_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZTYM_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZTYM_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZTYM_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZTYM_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZTYM_SPENT_USED_ZTYM = 14,                      // Coin has already been spend
    ZTYM_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZTYM_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZTYMOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzTYMTracker> zpivTracker;
    ...
    void setZWallet(CzTYMWallet* zwallet);
    CzTYMWallet* getZWallet();
    ...
    /// Get 10000 TYM output and keys which can be used for the Masternode
    ...
    /** zTYM reset */
    boost::signals2::signal<void()> NotifyzTYMReset;
