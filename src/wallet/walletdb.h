...
public:
    ...
    bool WriteZTYMSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZTYMSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZTYMSeed_deprecated(uint256& seed);
    bool EraseZTYMSeed();
    bool EraseZTYMSeed_deprecated();

    bool WriteZTYMCount(const uint32_t& nCount);
    bool ReadZTYMCount(uint32_t& nCount);
