...
CzTYMTracker::CzTYMTracker(std::string strWalletFile)
{
...
CzTYMTracker::~CzTYMTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzTYMTracker::Init()
{
...
bool CzTYMTracker::Archive(CMintMeta& meta)
{
...
bool CzTYMTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzTYMTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzTYMTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzTYMTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzTYMTracker::GetSerialHashes()
{
...
CAmount CzTYMTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzTYMTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzTYMTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzTYMTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzTYMTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzTYMTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzTYMTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzTYMTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzTYMTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzTYMTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzTYMTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzTYMWallet* zTYMWallet)
{
    bool iszTYMWalletInitialized = (NULL != zTYMWallet);
    CMintMeta meta;
    ...
    if (! iszTYMWalletInitialized)
        zTYMWallet = new CzTYMWallet(strWalletFile);
    meta.isSeedCorrect = zTYMWallet->CheckSeed(dMint);
    if (! iszTYMWalletInitialized)
        delete zTYMWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzTYMTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzTYMTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzTYMTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzTYMTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzTYMTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzTYMTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzTYMWallet* zTYMWallet = new CzTYMWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zTYMWallet);
        }
        delete zTYMWallet;
