...
CzTYMWallet::CzTYMWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZTYMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZTYMSeed_deprecated()) {
                    LogPrintf("%s: Updated zTYM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzTYMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZTYMCount(nCountLastUsed);
    else if (!walletdb.ReadZTYMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzTYMWallet::Lock()
{
    seedMaster = 0;
}

void CzTYMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzTYMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZTYM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzTYMWallet::LoadMintPoolFromDB()
{
...
void CzTYMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzTYMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzTYMWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzTYMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZTYM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzTYMWallet::SeedToZTYM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzTYMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzTYMWallet::GenerateDeterministicZTYM(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzTYMWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzTYMWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzTYMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
