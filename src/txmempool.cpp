...
void CTxMemPool::removeCoinbaseSpends(const CCoinsViewCache* pcoins, unsigned int nMemPoolHeight)
{
    ...
    for (std::map<uint256, CTxMemPoolEntry>::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        for (const CTxIn& txin : tx.vin) {
            ...
            if (coins->vout[txin.prevout.n].nValue == CollateralRequired(nMemPoolHeight)
                && IsImmatureCollateral(nMemPoolHeight, coins->nHeight))
            {
            ...
            if (IsMasternodePayment(coins->vout[txin.prevout.n].nValue, nMemPoolHeight)
                && nMemPoolHeight - coins->nHeight < (unsigned)Params().MN_REWARD_MATURITY())
            {
                LogPrintf("CTxMemPool immature MN reward tx removed %s  \n", tx.GetHash().ToString().c_str());
                transactionsToRemove.push_back(tx);
                break;
            }
        }
