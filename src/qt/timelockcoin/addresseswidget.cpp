...
#include "qt/timelockcoin/forms/ui_addresseswidget.h"
#include "qt/timelockcoin/addresslabelrow.h"
#include "qt/timelockcoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(timelockcoinGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination timelockcoinAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(timelockcoinAdd, label.toUtf8().constData(),
