...
#include "qt/timelockcoin/forms/ui_send.h"
#include "qt/timelockcoin/addnewcontactdialog.h"
#include "qt/timelockcoin/qtutils.h"
#include "qt/timelockcoin/sendchangeaddressdialog.h"
#include "qt/timelockcoin/optionbutton.h"
#include "qt/timelockcoin/sendconfirmdialog.h"
#include "qt/timelockcoin/myaddressrow.h"
#include "qt/timelockcoin/guitransactionsutils.h"
...
SendWidget::SendWidget(TimelockCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (TYM)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("TimelockCoin address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 TYM");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_TYM : AskPassphraseDialog::Context::Send_zTYM, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zTYM is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zTYM selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zTYM transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZTYM_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zTYM require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zTYM transaction
        ...
        emit message("zTYM transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isTYM){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zTYM in your balance to select."));
        }
    }
}
...
void SendWidget::onTYMSelected(bool _isTYM){
    isTYM = _isTYM;
    setCssProperty(coinIcon, _isTYM ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
