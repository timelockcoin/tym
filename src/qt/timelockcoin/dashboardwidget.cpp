...
#include "qt/timelockcoin/dashboardwidget.h"
#include "qt/timelockcoin/forms/ui_dashboardwidget.h"
#include "qt/timelockcoin/sendconfirmdialog.h"
#include "qt/timelockcoin/txrow.h"
...
DashboardWidget::DashboardWidget(timelockcoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareTYM, "square-chart-timelockcoin");
    setCssProperty(ui->labelTYM, "text-chart-timelockcoin");
    ...
    setCssProperty(ui->labelAmountTYM, "text-stake-timelockcoin-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &timelockcoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &timelockcoinGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &timelockcoinGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        qreal timelockcoin = 0;
        if (chartData->amountsByCache.contains(num)) {
            std::pair <qint64, qint64> pair = chartData->amountsByCache[num];
            timelockcoin = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalTYM += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesTYM.append(timelockcoin);

        int max = std::max(timelockcoin, timelockcoin);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(102, 141, 170));
    ...
    set0->append(chartData->valuesTYM);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalTYM > 0) {
        setCssProperty(ui->labelAmountTYM, "text-stake-timelockcoin");
    } else {
        setCssProperty(ui->labelAmountTYM, "text-stake-timelockcoin-disable");
    }
    forceUpdateStyle({ui->labelAmountTYM});
    ui->labelAmountTYM->setText(GUIUtil::formatBalance(chartData->totalTYM, nDisplayUnit));
