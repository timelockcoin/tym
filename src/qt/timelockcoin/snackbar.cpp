...
#include "qt/timelockcoin/forms/ui_snackbar.h"
#include "qt/timelockcoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(timelockcoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &timelockcoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
