...
#include "qt/timelockcoin/forms/ui_privacywidget.h"
#include "qt/timelockcoin/qtutils.h"
#include "guiutil.h"
#include "qt/timelockcoin/denomgenerationdialog.h"
#include "qt/timelockcoin/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(TimelockCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zTYM anonymizes your TYM by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zTYM or convert back to TYM"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 TYM ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zTYM");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zTYM");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zTYM");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zTYM");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zTYM");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zTYM");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zTYM");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zTYM");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zTYM Movements"));
    ...
    // Only Convert to TYM enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzTYM->setTitleClassAndText("btn-title-grey", "Total 0 zTYM");
    ui->btnTotalzTYM->setSubTitleClassAndText("text-subtitle", "Show denominations of zTYM owned.");
    ui->btnTotalzTYM->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select TYM outputs to mint into zTYM.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zTYM");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzTYM, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zTYM");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of TYM to mint into zTYM"));
    }else{
        btnText = tr("Convert back to TYM");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zTYM to convert back into TYM"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzTYM->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzTYM->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zTYM is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zTYM, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zTYM").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zTYM minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zTYM converted back to TYM"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any TYM to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTYM </b>";
    ...
    ui->btnTotalzTYM->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
