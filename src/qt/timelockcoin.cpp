...
#include "qt/timelockcoin/timelockcoingui.h"
...
#include "qt/timelockcoin/splash.h"
#include "qt/timelockcoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("timelockcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in timelockcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in timelockcoin.qrc)
...
/** Class encapsulating timelockcoin startup and shutdown.
...
/** Main timelockcoin application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (timelockcoinGUI)
...
{
    window = new timelockcoinGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &timelockcoinGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &timelockcoinGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(timelockcoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(timelockcoinGUI::DEFAULT_WALLET);
        ...
        // timelockcoin: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &timelockcoinGUI::handlePaymentRequest);
        connect(window, &timelockcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. TimelockCoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(timelockcoin_locale);
    Q_INIT_RESOURCE(timelockcoin);
    ...
    /// 6. Determine availability of data directory and parse timelockcoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("TimelockCoin"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("TimelockCoin"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // timelockcoin: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("TimelockCoin didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
