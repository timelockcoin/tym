...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::TYM);
    ...
    if (settings.contains("nAnonymizetimelockcoinAmount") || reset)
        SoftSetArg("-anonymizetimelockcoinamount", settings.value("nAnonymizetimelockcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in timelockcoin.cpp during shotdown to also remove the window positions
