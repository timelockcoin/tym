...
bool CTYMStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CTYMStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CTYMStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CTYMStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CTYMStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CTYMStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CTYMStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CTYMStake::GetUniqueness() const
{
...
CBlockIndex* CTYMStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CTYMStake::ContextCheck(int nHeight, uint32_t nTime)
{
