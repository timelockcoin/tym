...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("timelockcoin CoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("timelockcoin CoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("timelockcoin CoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("timelockcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running timelockcoin CoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("timelockcoin CoinMiner exception");
    } catch (...) {
        LogPrintf("timelockcoin CoinMiner exception");
    }

    LogPrintf("timelockcoin CoinMiner exiting\n");
}
