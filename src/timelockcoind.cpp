...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/timelockcoin.conf are parsed in qt/timelockcoin.cpp's main()
    ...
    try {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "timelockcoin:"))
                fCommandLine = true;

        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in timelockcoind anymore. Use the timelockcoin-cli utility instead.\n");
            exit(1);
        }
#ifndef WIN32
...
int main(int argc, char* argv[])
{
    ...
    // Connect timelockcoind signal handlers
    noui_connect();
